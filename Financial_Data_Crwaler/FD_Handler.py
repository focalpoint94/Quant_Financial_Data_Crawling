...
class FD_Handler():
    def __init__(self, code):
        self.code = code
        self.saved_dir = 'C:/Git/newFinance_workspace/Data_Handler/Financial Data/fsdata/'
        self.has_data = True
...
if __name__ == '__main__':
    saved_dir = 'C:/Git/newFinance_workspace/Data_Handler/Financial Data/fsdata/'
    code_list = os.listdir(saved_dir)
    for i, file in enumerate(code_list):
        code_list[i] = file.split('.')[0]
    year_list = [2009, 2014, 2019]
    #year_list = [2012, 2017]
    for year in year_list:
        for code in code_list:
            ...
            inc_kw_list = []
            exc_kw_list = ['비', '기타']
            k, v = FDH.get_value(year=str(year), label_list=label_list, inc_kw_list=inc_kw_list, exc_kw_list=exc_kw_list)

            if k != '유동자산' and k is not None:
                print(code)
                print(k)
